...
void setup() {
  ...
  Serial.println(" 11) Final 90° right (phase 4) + servo throw sequence, then stop.");
  Serial.println();
}
...
void updateAutoRoutine(unsigned long now) {
  switch (autoState) {
    ...
    case AUTO_THROW: {
      ...
      //  step 0: S1=30, S2=50, wait 1s
      //  step 1: S1=90, S2=100, wait 0.2s
      //  step 2: S3=0, done
      switch (throwStep) {
        case 0:
          // Move arm to pre-throw pose
          angle1 = 30;
          angle2 = 50;
          servo1.write(angle1);
          servo2.write(angle2);
          throwStartTime = now;
          throwStep = 1;
          Serial.println("Throw: S1->30, S2->50");
          break;
        ...
        case 1:
          if (now - throwStartTime >= 1000UL) { // 1 second
            ...
            throwStartTime = now;
            throwStep = 2;
            Serial.println("Throw: S1->90, S2->100");
          }
          break;
